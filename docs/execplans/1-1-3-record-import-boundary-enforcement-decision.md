...
uv tool run --python 3.14 \
  --from 'git+https://github.com/leynos/hecate.git@<sha>' hecate ...
```

and must never execute `import hecate`. Invoking `uv tool run --python 3.14`
...
the contract tests lock the policy, roadmap item 1.1.3 is marked done, all
local gates pass, CodeRabbit reports no unresolved in-scope concerns, and the
draft pull request is open.
...
```console
uv tool run --python 3.14 --from '<HECATE_SPEC>' \
  hecate check --config <fixture config> --format json
```

reading the pinned reference from the `HECATE_REF` environment variable
