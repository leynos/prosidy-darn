...
- [ ] 1.1.4. Record the profile rule-expression policy.
  ...
  - Status: implementation is in progress on branch
    `1-1-4-record-the-profile-rule-expression-policy`; branch and pull-request
    housekeeping are complete, but ADR-003 acceptance, documentation-contract
    tests, CodeRabbit review, and final quality gates remain open.
